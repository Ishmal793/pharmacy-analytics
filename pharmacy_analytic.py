...
# Shelf Analysis with Minute-Level Data
    ...
    if shelves:
        ...
        # Check if Shelf columns exist in filtered_data
        shelves = [col for col in ['Shelf 1', 'Shelf 2', 'Shelf 3', 'Shelf 4'] if col in filtered_data.columns]

        if shelves:
            # Melt operation for reshaping data
            melted_data = filtered_data.melt(
                id_vars=['Timestamp'],
                value_vars=shelves,
                var_name='Shelf',
                value_name='Shelf_Usage'  # Updated name to avoid clashes
            )

            # Add Current Customers and Current Visitors
            melted_data['Current Customers'] = filtered_data['Current Customers'].repeat(len(shelves)).reset_index(
                drop=True)
            melted_data['Current Visitors'] = filtered_data['Current Visitor'].repeat(len(shelves)).reset_index(
                drop=True)
            # Create a grouped bar chart
            fig2 = px.bar(
                melted_data,
                x='Shelf',
                y=['Current Customers', 'Current Visitors'],
                title="Shelf-wise Grouping of Current Customers and Current Visitors (Filtered Data)",
                labels={'value': 'Count', 'Shelf': 'Shelf Name'},
                barmode='group',
                color_discrete_sequence=["#1f77b4", "#ff7f0e"]  # Aesthetic colors for grouping
            )
            fig2.update_layout(
                plot_bgcolor="rgba(0,0,0,0)",
                paper_bgcolor="rgba(0,0,0,0)",
